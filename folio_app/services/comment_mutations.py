...
from folio_app.services.comment_queries import attach_comment_authors, can_reply_to_comment, can_reply_to_community_comment
...
def create_community_comment(post_id: str, author_id: str, body: str, parent_id: str | None = None) -> CommentResult:
    from folio_app.services.auth import ensure_authenticated_session
    ...
    if parent_id and not can_reply_to_community_comment(post_id, parent_id):
        return CommentResult(False, "답글을 남길 댓글을 확인할 수 없습니다.")
    ...
    try:
        ...
        logger.exception("Failed to create community comment")
        return CommentResult(False, "댓글 등록에 실패했습니다. 잠시 후 다시 시도하세요.")
