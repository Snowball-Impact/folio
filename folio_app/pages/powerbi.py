...
def render() -> None:
    ...
    learning_rows = _read_csv(LEARNING_CSV)
    update_video_rows = _read_csv(UPDATE_VIDEOS_CSV)
    learning_program_rows = _read_csv(LEARNING_PROGRAMS_CSV)
    community_rows = _read_csv(COMMUNITY_BLOG_CSV)

    topic = st.query_params.get("topic") or "news"
    _render_hero(_first(desktop_rows), topic=topic)
    if topic == "certifications":
        _render_certifications()
    elif topic == "learning":
        _render_learning_content(learning_rows, learning_program_rows)
    elif topic == "community":
        _render_community_content(community_rows)
    else:
        _render_news_board(update_rows, changelog_rows, update_video_rows)
...
def _community_groups(rows: list[dict[str, str]]) -> "OrderedDict[str, list[dict[str, str]]]":
...
def _community_card_html(row: dict[str, str]) -> str:
    ...
    published_at = _localize_date(row.get("published_at"))
    topic = row.get("topic") or "실무 팁"
...
def _render_learning_content(rows: list[dict[str, str]], program_rows: list[dict[str, str]]) -> None:
    if not rows:
        ...
        return
    categories = _learning_categories(rows)
    ...
    for tab, (category, category_rows) in zip(tabs, categories.items()):
        with tab:
            _render_learning_programs(_programs_for_category(program_rows, category))
...
def _programs_for_category(rows: list[dict[str, str]], category: str) -> list[dict[str, str]]:
...
def _learning_categories(rows: list[dict[str, str]]) -> "OrderedDict[str, list[dict[str, str]]]":
...
def _render_news_board(
    ...
    items = _build_news_items(update_rows, changelog_rows, update_video_rows)
    if not items:
...
def _news_video_html(row: dict[str, str] | None) -> str:
    ...
    href = html.escape(row.get("video_url") or "#", quote=True)
    thumbnail_url = html.escape(_learning_thumbnail_src(row) or _youtube_thumbnail_url(row), quote=True)
...
def _youtube_thumbnail_url(row: dict[str, str]) -> str:
