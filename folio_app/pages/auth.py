...
def _render_email_feedback(email: str, already_registered: bool) -> None:
    if not email:
        st.caption("로그인과 인증 메일 수신에 사용할 이메일을 입력하세요.")
        return
...
def render_login() -> None:
    render_hero(
        "Auth",
        "로그인",
        "프로젝트를 등록하고 내 포트폴리오를 관리하려면 로그인하세요.",
    )
...
def render_signup() -> None:
    render_hero(
        "Auth",
        "회원가입",
        "이메일 인증 후 로그인하면 프로젝트를 등록하고 포트폴리오를 관리할 수 있습니다.",
    )

    st.info("가입 후 받은 인증 메일을 확인한 다음, Login 메뉴에서 로그인하세요.")
    ...
    st.divider()
    st.markdown("### 인증 메일 다시 받기")
    st.caption("회원가입은 됐지만 인증 메일을 받지 못했거나 링크가 만료됐다면 다시 요청하세요.")
