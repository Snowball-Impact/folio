...
export type PolicyRequirementCandidate = { policy_type?: string | null; id?: string | null };
...
export function missingRequiredPolicyTypes(policies: PolicyRequirementCandidate[]) {
	const activeTypes = new Set(
		policies
			...
			.filter((policyType): policyType is RequiredPolicyType => policyType === 'terms' || policyType === 'privacy')
	);
