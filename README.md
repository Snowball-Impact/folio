...
- Thumbnail capture: `THUMBNAIL_CAPTURE_ENABLED`, `THUMBNAIL_CAPTURE_PROVIDER`, `THUMBNAIL_CAPTURE_ALLOWED_HOSTS`, `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_BROWSER_RENDERING_API_TOKEN` (`THUMBNAIL_CAPTURE_ALLOWED_HOSTS`에는 외부 캡처를 허용할 HTTPS 호스트를 쉼표로 구분해 등록. 현재 FOLIO 도메인·`APP_URL`·Cloudflare Preview origin은 자동 허용)
- Email: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM_EMAIL`, `SMTP_FROM_NAME`, `SMTP_USE_TLS`
- Analytics: `PUBLIC_GA_MEASUREMENT_ID`
