# Historical: Streamlit Community Cloud 배포와 캡처 실험

이 문서는 이전 Streamlit Community Cloud 배포 기준과 캡처 실험 기록이다. 현재 운영 배포 기준은 repository root의 SvelteKit 앱과 Cloudflare Pages이며, 새 배포 절차는 `docs/svelte/CLOUDFLARE_DEPLOYMENT.md`를 따른다.

이 문서의 `app.py`, `folio_app/`, `requirements*.txt`, `packages.txt` 경로는 로컬 백업 zip(`archive/streamlit_app_20260909.zip`)에 보관된 과거 소스를 가리킨다.
