...
- **오래된 서버 프로세스는 최신 코드를 가릴 수 있다.** Windows/Streamlit 포트 혼선을 줄이기 위해 `.streamlit/config.toml`은 `fileWatcherType = "none"`을 기본으로 둔다. 코드 변경 후에는 서버를 재시작하고, 반영이 이상하면 8501 리스너가 하나인지 확인한다.
