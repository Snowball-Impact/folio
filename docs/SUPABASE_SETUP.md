...
3. 이메일, 비밀번호, 이름, 소속을 입력해 회원가입합니다.
   - 소속은 개인, 학원, 교육과정, 학교, 기관, 회사명 등으로 입력할 수 있습니다.
   - 회원가입 화면의 성공 메시지는 메일 발송 자체를 보장하지 않고 요청 처리 여부만 안내합니다.
...
회원가입 후 인증 메일을 받지 못했다면 `Sign Up` 화면의 접힌 **인증 메일 다시 받기** 영역을 사용할 수 있습니다. 이 영역은 가입 성공 후 또는 이미 가입된 이메일을 입력했을 때만 표시됩니다.

중요한 흐름:

- 회원가입 버튼을 누르면 Supabase Auth 사용자와 `profiles` row가 먼저 생성됩니다.
- 이메일 인증은 그 다음 단계입니다.
- 앱은 `profiles` row가 있는 이메일의 재가입을 막습니다. `profiles` 조회가 실패하면 가입을 진행하지 않습니다.
- Supabase Auth가 기존 이메일을 `user.identities == []` 형태의 성공 응답처럼 돌려주는 경우도 "이미 가입된 이메일"로 차단합니다.
- 기존 Auth 사용자에 `profiles` row가 누락되고 공급자 응답에서도 중복 신호가 오지 않는 예외 상황은 앱의 anon 권한만으로 확정할 수 없으므로, DB 트리거와 `profiles` row 생성을 반드시 확인합니다.
- Supabase Auth는 이메일 열거 방지를 위해 기존 이메일에도 성공처럼 응답할 수 있습니다. 재발송 문구의 "요청 처리"는 실제 메일 발송을 보장한다는 뜻이 아닙니다.
- 인증을 완료하지 못한 사용자는 재가입이 아니라 **인증 메일 재발송**을 사용합니다.

1. `Login` 화면에서 회원가입 링크를 눌러 `Sign Up` 화면으로 이동합니다.
2. 가입한 이메일을 입력해 기존 계정 안내가 표시되면 `인증 메일 다시 받기` 영역을 펼칩니다.
