...
### 개발 서버 파일 감시 설정 (2026-07-07)

`.streamlit/config.toml`의 `fileWatcherType`은 `"none"`으로 둔다. Windows + Streamlit 1.58 환경에서 자동 reload 중 오래된 Uvicorn 프로세스가 8501 포트를 계속 잡아 최신 코드가 가려지는 혼선이 있었고, `auto`로 다시 시도했을 때도 회원가입 화면 변경이 즉시 반영되지 않아 수동 재시작 방식으로 되돌렸다.

코드/CSS 변경 후에는 Streamlit 서버를 재시작한다. 반영이 이상하면 먼저 `netstat -ano -p tcp`로 `0.0.0.0:8501` 리스너가 하나인지 확인한다.
