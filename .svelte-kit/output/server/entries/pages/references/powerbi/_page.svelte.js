...
//#region routes/references/powerbi/+page.svelte
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<!--]--></nav></div></section> `);
		if (projects().length > 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<section class="reference-toolbar" aria-label="레퍼런스 정렬"><span>정렬</span> <nav><!--[-->`);
		...
		if (data.error) {
			...
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<div class="empty-panel">아직 표시할 Power BI 레퍼런스가 없습니다. 공개 레퍼런스가 준비되면 이곳에서 바로 살펴볼 수 있습니다.</div>`);
		}
		$$renderer.push(`<!--]-->`);
	});
}
