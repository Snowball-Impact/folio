...
import { frameSourceFromUrl, securityHeaders } from '../../src/lib/server/security-headers.ts';
...
test('allows required Supabase, RUM, Power BI, and Fabric origins in CSP', () => {
	const headers = securityHeaders({
...
test('allows nonce-based SvelteKit inline scripts without enabling all inline scripts', () => {
	const csp = securityHeaders({ scriptNonce: 'test-nonce' })['Content-Security-Policy'];
