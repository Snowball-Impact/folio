...
const PROJECT_HTML_ALLOWED_TAGS = new Set([
	'a',
	'b',
	'blockquote',
	'br',
...
export function sanitizeProjectHtml(value: string | null | undefined, options: { allowLocalImages?: boolean } = {}) {
	const raw = String(value ?? '').trim();
	if (!raw) {
		return '';
	}
	const source = /<[a-z][\s\S]*>/i.test(raw) ? raw : plainTextToParagraphHtml(raw);
	return source
		.replace(/[\s\S]*?-->/g, '')
		.replace(/<\s*(script|style|iframe|object|embed|form|input|button|svg|math|meta|link)[\s\S]*?<\s*\/\s*\1\s*>/gi, '')
		.replace(/<\s*(script|style|iframe|object|embed|form|input|button|svg|math|meta|link)[^>]*\/?>/gi, '')
		.replace(/<\/?([a-zA-Z0-9-]+)([^>]*)>/g, (match, tagName, attrs) => {
			const tag = String(tagName).toLowerCase();
			if (!PROJECT_HTML_ALLOWED_TAGS.has(tag)) {
				return '';
			}
			if (match.startsWith('</')) {
				return `</${tag}>`;
			...
			if (tag === 'span') {
				return `<span${safeSpanStyle(String(attrs ?? ''))}>`;
			}
			if (tag === 'mark') {
				return `<mark${safeMarkStyle(String(attrs ?? ''))}>`;
			}
...
function safeSpanStyle(attrs: string) {
	const styleMatch = attrs.match(/\bstyle\s*=\s*(?:"([^"]*)"|'([^']*)')/i);
	const style = (styleMatch?.[1] ?? styleMatch?.[2] ?? '').trim();
	const colorMatch = style.match(/(?:^|;)\s*color\s*:\s*(#[0-9a-f]{3,8}|rgba?\([^)]*\))\s*(?:;|$)/i);
	const color = colorMatch?.[1] ?? '';
	const styles = isSafeCssColor(color) ? [`color: ${color}`] : [];
	const familyMatch = style.match(/(?:^|;)\s*font-family\s*:\s*([^;]+)\s*(?:;|$)/i);
	const family = (familyMatch?.[1] ?? '').replace(/["']/g, '').trim().toLowerCase();
	...
	if (sizeMatch?.[1]) {
		styles.push(`font-size: ${sizeMatch[1]}`);
	}
	return styles.length ? ` style="${escapeHtml(styles.join('; '))}"` : '';
}

function safeIndent(attrs: string) {
...
function safeImageSrc(attrs: string, allowLocalImages = false) {
...
function safeAttribute(attrs: string, name: string) {
	const expression = new RegExp(`\\b${name}\\s*=\\s*(?:"([^"]*)"|'([^']*)'|([^\\s>]+))`, 'i');
	const match = attrs.match(expression);
	return (match?.[1] ?? match?.[2] ?? match?.[3] ?? '').trim();
}

function isMathNode(attrs: string) {
	return Boolean(mathNodeType(attrs));
}

function mathNodeType(attrs: string) {
	const type = safeAttribute(attrs, 'data-type');
	return type === 'inline-math' || type === 'block-math' ? type : '';
}

function safeMarkStyle(attrs: string) {
	const styleMatch = attrs.match(/\bstyle\s*=\s*(?:"([^"]*)"|'([^']*)')/i);
	const style = (styleMatch?.[1] ?? styleMatch?.[2] ?? '').trim();
	const backgroundMatch = style.match(/(?:^|;)\s*background-color\s*:\s*(#[0-9a-f]{3,8}|rgba?\([^)]*\))\s*(?:;|$)/i);
